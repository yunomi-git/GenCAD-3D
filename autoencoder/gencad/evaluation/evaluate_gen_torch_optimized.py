...
def collect_src_pcs(args):
    ...
    print('#'*50)
    print(f"total valid CAD: {len(all_paths)}")
    print('#'*50)
...
def process_iteration(args, iteration, result_queue):
...
def main():
    ...
    print(f"n_test: {args.n_test}, multiplier: {args.multi}, repeat times: {args.times}")

    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_ids)

    if args.output is None:
        args.output = f"{args.src}_eval_gen.txt"

    result_queue = mp.Queue()
    processes = []

    for i in range(args.times):
        p = mp.Process(target=process_iteration, args=(args, i, result_queue))
        p.start()
        processes.append(p)
    ...
    avg_result = {f"avg-{k}": np.mean([x[k] for x in results]) for k in results[0].keys()}
