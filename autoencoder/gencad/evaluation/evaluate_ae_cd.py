...
def process_one(path):
    ...
    if not os.path.exists(gt_pc_path):
        print("gt pc not found:", gt_pc_path)
        return None
...
def run_eval_ae_cd(src, n_points=2000, num=-1, parallel=False):
    print("#"*25)
    print('running accuracy test: chamfer distance')
    print("#"*25)

    filepaths = sorted(glob.glob(os.path.join(src, "*.h5")))
    if num != -1:
        filepaths = filepaths[:num]
