...
# == Pytorch things
def mesh_to_np(verts, faces=None):
    verts_np =  verts.detach().to(torch.device('cpu')).numpy().astype(np.float32)

    if faces is not None:
        faces_np = faces.detach().to(torch.device('cpu')).numpy().astype(np.int64)
        return verts_np, faces_np

    return verts_np

def mesh_to_tensor(verts, faces=None):
    verts_tens = torch.Tensor(verts).type(torch.float32)

    if faces is not None:
        faces_tens = torch.Tensor(faces).type(torch.int64)
        return verts_tens, faces_tens

    return verts_tens

def to_tensor(x, is_int):
    # int or float
    if is_int:
