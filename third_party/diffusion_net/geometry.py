...
import third_party.diffusion_net.utils as utils
...
def face_coords(verts, faces):
    if (faces > len(verts)).any():
        print("Length violated")
    coords = verts[faces]
    return coords
...
def mesh_vertex_normals(verts, faces):
    # numpy in / out
    verts_tens, face_tens = utils.mesh_to_tensor(verts, faces)
    face_n = toNP(face_normals(verts_tens, face_tens))  # ugly torch <---> numpy
...
def compute_operators(verts, faces, k_eig, normals=None):
    ...
    if k_eig > 0:
        ...
        failcount = 0
        while True:
            try: #TODO should I save the new eigenvalue?
                k_to_use = k_eig
                if k_to_use >= L_eigsh.shape[0]:
                    k_to_use = L_eigsh.shape[0] - 1
                ...
                # if np.isnan(L_eigsh).any() or np.isinf(L_eigsh).any():
                #     print("--- spectral decpomposition: NaN or Inf in L_eigsh")
                #     raise ValueError("failed to compute eigendecomp")
                evals_np, evecs_np = sla.eigsh(L_eigsh, k=k_to_use, M=Mmat, sigma=eigs_sigma, maxiter=500) # TODO tune maxiter
                # This returns an ARGPACK Error if failed to converge
                ...
                print("diffusion-net/geometry error:")
                print(e)
                if(failcount >= 0):
                    raise ValueError("failed to compute eigendecomp")
...
def get_operators_np(verts, faces, k_eig=128, op_cache_dir=None, normals=None, overwrite_cache=False):
    """
    ...
    if op_cache_dir is not None:
        ...
        while True:
            ...
            try:
                ...
                print("  cache miss -- constructing operators, ", hash_key_str)
                break
...
def get_operators(verts, faces, k_eig=128, op_cache_dir=None, normals=None, overwrite_cache=False, is_np=False, device=None):
    """
    See documentation for compute_operators(). This essentailly just wraps a call to compute_operators, using a cache if possible.
    All arrays are always computed using double precision for stability, then truncated to single precision floats to store on disk, and finally returned as a tensor with dtype/device matching the `verts` input.
    """
    ...
    if is_np:
        ...
        verts_redo, faces_redo = utils.mesh_to_np(*utils.mesh_to_tensor(verts, faces))
    else:
        device = verts.device
        dtype = verts.dtype
