...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, d_out, dropout, d_layers, dim_t = 128):
        super().__init__()
        self.dim_t = dim_t

        self.mlp = MLP.make_baseline(d_in, d_layers, dropout, d_out)
...
class ResNetDiffusion(nn.Module):
    def __init__(self, d_in, n_blocks, d_main, d_hidden, dropout_first, dropout_second, d_out, dim_t = 256):
        super().__init__()
        self.dim_t = dim_t

        self.resnet = ResNet.make_baseline(d_in, n_blocks, d_main, d_hidden, dropout_first, dropout_second, d_out)
        self.proj = nn.Linear(2* d_in, dim_t)
